...
describe('facialSentimentTask', () => {
  ...
  it('analyzes video, stores facialSentimentResults, and rebuilds analytics/sentiment', async () => {
    ...
    expect(mockAnswerUpdate).toHaveBeenCalledWith({
      'taskAnswers.evaluator-1.tasks.0.facialSentimentResults': emotionsPayload,
    })
  ...
  it('overwrites facialSentimentResults on re-analyze', async () => {
    mockAnswerGet.mockResolvedValue(
      snap(true, {
        studyId: 'study-1',
        taskAnswers: {
