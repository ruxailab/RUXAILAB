...
export function roleFromAccessLevel(accessLevel) {
  if (accessLevel === ACCESS_LEVEL.OBSERVATOR) return VIDEO_CALL_ROLES.OBSERVATOR
  if (accessLevel === ACCESS_LEVEL.ADMIN) return VIDEO_CALL_ROLES.MODERATOR
...
export function mediaCapabilitiesFor(accessLevel) {
  const isObservator = accessLevel === ACCESS_LEVEL.OBSERVATOR
  return {
