<template>
  <v-form ref="form" v-model="valid">
    <!-- Information header -->
    <v-card variant="elevated" class="mb-6 bg-white sus-card">
      <v-card-text class="text-center">
        ...
        <p class="text-body-2 text-grey-darken-3 mb-2">
          ...
          <strong>10 questions</strong>. Rate each statement on a scale from
          <strong>Strongly Disagree</strong> to <strong>Strongly Agree</strong>.
        </p>
        <p class="text-body-2 text-grey-darken-3">
          Your honest feedback helps us understand the usability of the system.
...
.sus-card :deep(.v-card-text) {
  background: #fff !important;
}

.sus-card :deep(.v-radio-group) {
  background: #fff !important;
}

.sus-card :deep(.v-field) {
  background: #fff !important;
}
</style>
