...
watchEffect(() => {
  ...
  const tamAnswers = localTestAnswer?.tasks?.[index]?.tamAnswers
  ...
  if (task?.taskType === 'sus') {
  ...
  } else if (task?.taskType === 'tam-1') {
    // TAM-1 has 12 questions (6 for Perceived Usefulness + 6 for Perceived Ease of Use)
    const puAnswers = tamAnswers?.perceivedUsefulness || []
    const peuAnswers = tamAnswers?.perceivedEaseOfUse || []
    const validPU = puAnswers.filter((v) => typeof v === 'number').length
    const validPEU = peuAnswers.filter((v) => typeof v === 'number').length
    doneTaskDisabled.value = validPU < 6 || validPEU < 6
    console.log('TAM-1 respostas válidas:', validPU, validPEU)
  } else if (task?.taskType === 'tam-2') {
    // TAM-2 has all TAM-1 constructs plus additional ones
    const puAnswers = tamAnswers?.perceivedUsefulness || []
    const peuAnswers = tamAnswers?.perceivedEaseOfUse || []
    const snAnswers = tamAnswers?.subjectiveNorm || []
    const imageAnswers = tamAnswers?.image || []
    const jrAnswers = tamAnswers?.jobRelevance || []
    const oqAnswers = tamAnswers?.outputQuality || []
    const rdAnswers = tamAnswers?.resultDemonstrability || []

    const validPU = puAnswers.filter((v) => typeof v === 'number').length
    const validPEU = peuAnswers.filter((v) => typeof v === 'number').length
    const validSN = snAnswers.filter((v) => typeof v === 'number').length
    const validImage = imageAnswers.filter((v) => typeof v === 'number').length
    const validJR = jrAnswers.filter((v) => typeof v === 'number').length
    const validOQ = oqAnswers.filter((v) => typeof v === 'number').length
    const validRD = rdAnswers.filter((v) => typeof v === 'number').length

    doneTaskDisabled.value =
      validPU < 6 ||
      validPEU < 6 ||
      validSN < 4 ||
      validImage < 3 ||
    ...
    // TAM-3 has all TAM-2 constructs plus additional ones
    const puAnswers = tamAnswers?.perceivedUsefulness || []
    const peuAnswers = tamAnswers?.perceivedEaseOfUse || []
    const snAnswers = tamAnswers?.subjectiveNorm || []
    const imageAnswers = tamAnswers?.image || []
    ...
    const oqAnswers = tamAnswers?.outputQuality || []
    const rdAnswers = tamAnswers?.resultDemonstrability || []
    const cseAnswers = tamAnswers?.computerSelfEfficacy || []
    const pecAnswers = tamAnswers?.perceptionOfExternalControl || []
    const caAnswers = tamAnswers?.computerAnxiety || []
    const cpAnswers = tamAnswers?.computerPlayfulness || []
    const peAnswers = tamAnswers?.perceivedEnjoyment || []
    const ouAnswers = tamAnswers?.objectiveUsability || []
    ...
    const validOQ = oqAnswers.filter((v) => typeof v === 'number').length
    const validRD = rdAnswers.filter((v) => typeof v === 'number').length
    const validCSE = cseAnswers.filter((v) => typeof v === 'number').length
    const validPEC = pecAnswers.filter((v) => typeof v === 'number').length
    const validCA = caAnswers.filter((v) => typeof v === 'number').length
    const validCP = cpAnswers.filter((v) => typeof v === 'number').length
    const validPE = peAnswers.filter((v) => typeof v === 'number').length
    const validOU = ouAnswers.filter((v) => typeof v === 'number').length
