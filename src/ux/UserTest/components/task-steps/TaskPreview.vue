  ...
  <div class="task-preview">
    ...
    <v-card class="task-summary-card mb-6" elevation="2">
      <v-card-title class="d-flex align-center bg-primary text-white">
        ...
        {{ $t('UserTestTaskDialog.preview.taskSummary') }}
      </v-card-title>
        ...
        <v-row>
          <v-col cols="12" md="8">
            <div class="task-details">
              <h4 class="text-h6 font-weight-bold mb-3">
                {{
                  (task && task.taskName) ||
                  $t('UserTestTaskDialog.preview.untitledTask')
                }}
              </h4>

              <div class="task-description mb-4">
                <div class="text-body-2 text-grey-darken-1 mb-2">
                  {{ $t('UserTestTaskDialog.fields.taskDescription') }}:
                </div>
                <div class="description-content">
                  {{
                    (task && task.taskDescription) ||
                    $t('UserTestTaskDialog.preview.noDescription')
                  }}
                </div>
              </div>
              ...
              <div v-if="task && task.taskTip" class="task-tip mb-4">
                <div class="text-body-2 text-grey-darken-1 mb-2">
                  {{ $t('UserTestTaskDialog.preview.participantTip') }}:
                </div>
                <v-alert
                  type="info"
                  variant="tonal"
              ...
              <div v-if="task && task.taskLink" class="task-link mb-4">
                <div class="text-body-2 text-grey-darken-1 mb-2">
                  {{ $t('UserTestTaskDialog.preview.taskUrl') }}:
                </div>
                <v-chip
                  :href="task.taskLink"
                  target="_blank"
          ...
          <v-col cols="12" md="4">
            <div class="task-config">
              ...
              >
                <div class="text-body-2 text-grey-darken-1 mb-2">
                  {{ $t('UserTestTaskDialog.preview.recordingFeatures') }}:
                </div>
              ...
              <div v-if="task && task.postQuestion" class="config-section mb-4">
                <div class="text-body-2 text-grey-darken-1 mb-2">
                  {{ $t('UserTestTaskDialog.preview.postTaskQuestion') }}:
                </div>
              ...
              <div v-if="task && task.postForm" class="config-section mb-4">
                <div class="text-body-2 text-grey-darken-1 mb-2">
                  {{ $t('UserTestTaskDialog.preview.postTaskForm') }}:
                </div>
                <v-chip
                  :href="task.postForm"
                  target="_blank"
                  ...
                  class="text-decoration-none"
                >
                  {{ $t('UserTestTaskDialog.preview.externalForm') }}
                </v-chip>
              </div>
            </div>
          </v-col>
    ...
    >
      <v-alert-title>
        {{
          isTaskComplete
            ? $t('UserTestTaskDialog.preview.taskReady')
            : $t('UserTestTaskDialog.preview.reviewRequired')
        }}
      </v-alert-title>
      <div class="text-body-2 mt-2">
        {{
          isTaskComplete
            ? $t('UserTestTaskDialog.preview.taskReadyDesc')
            : $t('UserTestTaskDialog.preview.reviewRequiredDesc')
        }}
      </div>
    </v-alert>
  </div>
...
const getAnswerTypeLabel = (type) => {
  if (!type) return t('UserTestTaskDialog.preview.untitledTask') // Or something like 'No Type Selected'
  const labels = {
    'no-answer': t('UserTestTaskDialog.answerTypes.noAnswer'),
    'text-area': t('UserTestTaskDialog.answerTypes.textArea'),
    'post-test': t('UserTestTaskDialog.answerTypes.postTest'),
    'post-form': t('UserTestTaskDialog.answerTypes.postForm'),
    'nasa-tlx': t('UserTestTaskDialog.answerTypes.nasaTlx'),
    sus: t('UserTestTaskDialog.answerTypes.sus'),
    'tam-1': t('UserTestTaskDialog.answerTypes.tam1'),
    'tam-2': t('UserTestTaskDialog.answerTypes.tam2'),
    'tam-3': t('UserTestTaskDialog.answerTypes.tam3'),
    sart: t('UserTestTaskDialog.answerTypes.sart'),
  }
