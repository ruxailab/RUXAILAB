<template>
  <div class="bg-background">
          ...
          <v-row dense>
            ...
            >
              ...
              <v-text-field
                ...
                clearable
                class="filter-field"
                @input="onFreeTextFilter(def.index)"
              />
            </v-col>
          </v-row>
        </div>
    ...
    <v-row class="">
      <v-col :cols="showTAM ? 12 : 12" :md="showTAM ? 3 : 4">
        <UxMetricCard
      ...
      <v-col v-if="showTAM" cols="12" md="3">
        <UxMetricCard
          :value="`${calculateTAMAcceptance().toFixed(1)}/7`"
          label="TAM Acceptance"
          color="purple"
          icon="mdi-thumb-up"
          description="Technology Acceptance Model average score across all TAM tasks"
          :progress="(calculateTAMAcceptance() / 7) * 100"
        />
      </v-col>
    </v-row>
...
const calculateTAMAcceptance = () => {
  if (!filteredSessions.value.length) return 0

  let totalAcceptance = 0
  let tamResponseCount = 0

  filteredSessions.value.forEach((answer) => {
    if (answer.tasks && typeof answer.tasks === 'object') {
      const tamTasks = Object.values(answer.tasks).filter((task) => {
        const taskType = testStructure.value?.userTasks?.find(
          (t) => t.taskId === task.taskId,
        )?.taskType
        return taskType && taskType.startsWith('tam-') && task.tamAnswers
      })

      tamTasks.forEach((task) => {
        try {
          const tamScores = calculateTAMScores(task.tamAnswers)
          const overallAcceptance = calculateOverallAcceptance(
            tamScores,
            task.tamAnswers.version,
          )
          totalAcceptance += overallAcceptance
          tamResponseCount++
        } catch (error) {
          console.warn('Error calculating TAM scores for task:', error)
        }
      })
    }
  })

  return tamResponseCount === 0 ? 0 : totalAcceptance / tamResponseCount
}
...
const calculateSatisfaction = () => {
  ...
  filteredSessions.value.forEach((answer) => {
    let sessionSatisfaction = null

    // First, try to get TAM-based satisfaction if available
    if (answer.tasks && typeof answer.tasks === 'object') {
      const tamTasks = Object.values(answer.tasks).filter((task) => {
        const taskType = testStructure.value?.userTasks?.find(
          (t) => t.taskId === task.taskId,
      ...
      if (tamTasks.length > 0) {
        // Calculate average TAM acceptance across all TAM tasks for this session
        let tamAcceptanceSum = 0
        let tamTaskCount = 0
        ...
        tamTasks.forEach((task) => {
          try {
            ...
            // Convert TAM 1-7 scale to 1-5 scale for consistency with other satisfaction metrics
            tamAcceptanceSum += (overallAcceptance - 1) * (4 / 6) + 1
            tamTaskCount++
          } catch (error) {
            console.warn('Error calculating TAM scores for task:', error)
          }
        ...
        if (tamTaskCount > 0) {
          sessionSatisfaction = tamAcceptanceSum / tamTaskCount
        }
      }
    }

    // Fall back to existing satisfaction data or simulated rating
    if (sessionSatisfaction === null) {
      if (answer.satisfaction && typeof answer.satisfaction === 'number') {
        sessionSatisfaction = answer.satisfaction
      } else {
