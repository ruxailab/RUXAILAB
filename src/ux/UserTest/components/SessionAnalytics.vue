<template>
        ...
        <v-col class="ma-0 pa-1 answer-list" cols="9">
          <div v-if="selectedUserId">
            <h3 class="text-h6 font-weight-bold mb-4">
              {{ $t('sessionAnalytics.tasksForUser', { id: selectedUserId }) }}
            </h3>
            <v-list>
              <v-list-item
          ...
          <div v-else>
            <p class="text-grey">
              {{ $t('sessionAnalytics.selectParticipant') }}
            </p>
          </div>
        </v-col>
      </v-row>
  ...
  <SessionAnalyticsDialog
    v-model="taskDialog"
    :user-id="selectedUserId ? String(selectedUserId) : ''"
    :selected-task-name="selectedTask ? selectedTask.taskName : ''"
    :task-id="selectedTask ? selectedTask.taskId : null"
  />
...
export default {
  ...
  data() {
    return {
      ...
      mockTranscript:
        'El usuario realizó la tarea correctamente y luego revisó el panel de configuración. El moderador preguntó sobre la experiencia y el usuario respondió positivamente.',
      mockEyeTracking: {
        accuracy: 92,
        fixations: 34,
      },
