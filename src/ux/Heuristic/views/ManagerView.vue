...
const navigator = computed(() => {
  ...
  const items = [
    { title: 'Manager', icon: 'mdi-home', path: `/heuristic/manager/${route.params.id}` }
  ]
  ...
  if (accessLevel.value === 0) {
    items.push(
      { title: 'Test', icon: 'mdi-file-document-edit', path: `/heuristic/edit/${test.value.id}` },
      { title: 'Preview', icon: 'mdi-file-eye', path: `/testview/${test.value.id}` },
      { title: 'Reports', icon: 'mdi-book-multiple', path: `/heuristic/report/${test.value.id}` },
      { title: 'Answers', icon: 'mdi-order-bool-ascending-variant', path: `/heuristic/answer/${test.value.id}` },
      { title: 'Final Report', icon: 'mdi-file-document', path: `/heuristic/finalreport/${test.value.id}` },
      { title: 'Cooperators', icon: 'mdi-account-group', path: `/heuristic/cooperators/${test.value.id}` },
      { title: 'Settings', icon: 'mdi-cog', path: `/heuristic/settings/${test.value.id}` }
    )
  }
  ...
  return [
    {
      ...
      path: `/heuristic/edit/${test.value.id}`,
    },
    {
      ...
      path: `/heuristic/cooperators/${test.value.cooperators}`,
    },
  ]
})
  ...
  return [
    {
      ...
      path: `/heuristic/report/${test.value.answersDocId}`,
    },
    {
      ...
      path: `/heuristic/answer/${test.value.answersDocId}`,
    },
  ]
})
