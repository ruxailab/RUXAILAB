...
const optionsWithFormattedValue = computed(() => {
  // Follow heuristics pattern: prefer Heuristic store when available, fallback to Study store otherwise
  const heuristicOptions = store.getters.testOptions
  const studyOptions = store.state.Tests?.Test?.testOptions || []

  const source = Array.isArray(heuristicOptions)
    ? heuristicOptions
    : studyOptions
...
const updateOptions = (newOption) => {
  ...
  // Follow heuristics pattern: get current options, modify, then dispatch
  const currentOptions = [...(store.getters.testOptions || [])]
  if (editIndex.value === -1) {
    ...
    currentOptions[editIndex.value] = {
      ...newOption,
      timestamp: currentOptions[editIndex.value].timestamp,
    }
    editIndex.value = -1
  }

  // Dispatch to Heuristic store (like heuristics does)
  store.dispatch('setTestOptions', currentOptions)
  resetForm()
  emitChange()
}
...
const editItem = (item) => {
  if (props.isTemplate) return
  const currentOptions = store.getters.testOptions || []
  editIndex.value = currentOptions.findIndex(
    (opt) => opt.timestamp === item.timestamp,
  )
  option.value = { ...currentOptions[editIndex.value] }
