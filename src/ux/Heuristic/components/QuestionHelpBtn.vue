<template>
  <div>
    <v-tooltip v-if="question.descriptions.length > 0" location="bottom">
    ...
    <v-dialog v-model="dialog" width="85%">
      <v-card>
        <div style="display: flex; justify-content: flex-end">
          <v-btn size="small" icon class="ma-1" @click="dialog = false">
        ...
        <div v-for="(description, i) in question.descriptions" :key="i">
          ...
          <TextBox class="mx-4" :text="description.text" :editable="false" />
        </div>
      </v-card>
    </v-dialog>
  </div>
