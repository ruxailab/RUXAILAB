const state = {
  ...
  completedRules: [],
  notes: {},
  ruleAssessments: {} // { ruleId: { severity: '', status: '', notes: '' } }
}
...
const getters = {
  ...
  getRuleAssessment: (state) => (ruleId) => {
    return state.ruleAssessments[ruleId] || {
      severity: '',
      status: '',
      notes: state.notes[ruleId] || ''
    }
  },
  
  // Get notes for a rule (kept for backward compatibility)
  getRuleNotes: (state) => (ruleId) => {
    return state.ruleAssessments[ruleId]?.notes || state.notes[ruleId] || ''
  },
...
const mutations = {
  ...
  UPDATE_RULE_NOTES(state, { ruleId, notes }) {
    ...
    // Also update in ruleAssessments for backward compatibility
    if (!state.ruleAssessments[ruleId]) {
      state.ruleAssessments[ruleId] = {}
    }
    state.ruleAssessments[ruleId].notes = notes
  },
...
const actions = {
  ...
  updateRuleAssessment({ commit }, { ruleId, severity, status, notes }) {
    ...
    // Also update notes separately for backward compatibility
    if (notes !== undefined) {
