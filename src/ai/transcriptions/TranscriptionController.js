...
export default class TranscriptionController extends Controller {
  ...
  async create(transcriptionData) {
    ...
    try {
      ...
      const conditions = [
        {
          field: 'answersDocId',
          condition: '==',
          value: transcriptionData.answersDocId,
        },
        {
          field: 'userDocId',
          condition: '==',
          value: transcriptionData.userDocId,
        },
        { field: 'taskId', condition: '==', value: transcriptionData.taskId },
      ]
      const result = await super.queryWithMultipleConditions(
        COLLECTION,
        conditions,
      )
      if (result.docs.length > 0) {
        ...
        throw new Error(
          `Document with answersDocId ${transcriptionData.answersDocId}, userDocId ${transcriptionData.userDocId}, and taskId ${transcriptionData.taskId} already exists.`,
        )
      } else {
   ...
   * @throws {Error} - Throws an error if there is an issue retrieving the document from Firestore.
   */
  async getByAnswersDocIdandUserDocIdandTaskId(
    answersDocId,
    userDocId,
    taskId,
  ) {
    ...
    const transcriptions = result.docs.map((doc) =>
