<template>
    <div class="recent-activity-list">
        <div v-if="activities.length === 0" class="empty-state text-center py-4">
            ...
            <p class="text-body-2 text-grey-darken-1">No recent activity</p>
        </div>
            ...
            <div v-for="(activity, index) in activities" :key="activity.id" class="activity-item"
                ...
                <div class="activity-indicator">
                    <div class="activity-icon" :class="`bg-${activity.color}`">
                        <v-icon :icon="activity.icon" size="16" color="white"></v-icon>
                    </div>
        ...
        <div v-if="activities.length > 0" class="mt-4 text-center">
            <v-btn variant="text" color="primary" size="small" @click="$router.push('/admin?section=notifications')">
                View All Activity
            </v-btn>
        </div>
    </div>
