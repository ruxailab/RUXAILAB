    ...
    <ShowInfo :title="testTitle + ' - ' + preTestTitle">
        <template #content>
            <div class="test-content pa-4 rounded-xl">
                <div class="rich-text mb-6 pa-4" v-html="consentText" />
                <v-row justify="center">
                    <v-col cols="12" md="6">
                        <v-text-field :model-value="localFullName" @update:modelValue="onFullNameInput"
                            label="Full Name" variant="outlined" density="comfortable"
                            :rules="[v => !!v || 'Name is required']" />
                    </v-col>
                </v-row>
                <v-row justify="center">
                    <v-col cols="12" md="6">
                        <v-radio-group :model-value="localConsentCompleted" @update:modelValue="onConsentInput" inline>
                ...
                <v-row justify="center" class="mt-4">
                    <v-col cols="12" md="6" class="text-center">
                        <v-btn color="primary" variant="flat" block :disabled="!localConsentCompleted || !localFullName"
                            @click="$emit('continue')">
                            Continue
                        </v-btn>
                    </v-col>
