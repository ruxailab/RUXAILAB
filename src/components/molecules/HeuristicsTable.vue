            ...
            <v-col cols="10">
                <v-card>               
                    ...
                    >
                        <template v-slot:top>
                        <v-toolbar flat>
                            <v-toolbar-title><b>Current Heuristics</b></v-toolbar-title>
                            <v-spacer></v-spacer>
                            <AddHeurBtn
                            :heuris="heuris"
                            :dialog="dialog"
                        ...
                        <td :colspan="headers.length" class="pa-3">
                            ...
                            <div class="caption" v-if="item.questions.length > 0">
                                <v-list>
                                    ...
                                    <v-list-item
                                    v-for="(question, n) in item.questions"
                                    :key="n"
                                    inactive
                                    >   
                        ...
                        <template v-slot:item.actions="{ item }"> <!-- table actions -->
                            <v-btn icon small class="mr-2" @click="editItem(item)"><v-icon small>mdi-pencil</v-icon></v-btn>
                            <v-btn icon small @click="deleteItem(item)"><v-icon small>mdi-delete</v-icon></v-btn>
                        </template>
                    </v-data-table>
                </v-card>
            </v-col>
