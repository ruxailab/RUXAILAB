<template>
  <v-container>
    <!-- Create Dialog -->
    <CreateVariable v-model:dialog="dialog" :title="$t('UserTestTable.buttons.createNewVariable')"
      @close="dialog = false" @save="saveNewItem" />

    <v-row justify="center">
      <v-col cols="10">
        <v-expansion-panels v-if="value.length > 0"
          style="z-index: auto; border-radius: 20px; border: 1px solid rgba(249, 152, 38, 0.49);">
          <v-expansion-panel v-for="(item, i) of value" :key="i" class="mb-1" style="border-radius: 20px;">
            ...
            <v-expansion-panel-text>
              <v-form>
                ...
                <div>
                  <v-text-field v-for="(field, index) of item.selectionFields" :key="index"
                    v-model="item.selectionFields[index]" :label="$t('UserTestTable.inputs.selection')">
                    <template #append>
                  ...
                  <div v-if="item.selectionField">
                    <p>
                      Add Option
                      <v-icon class="ml-1" @click="newSelection(i)">
                        mdi-plus
                      </v-icon>
                    </p>
                  </div>
        ...
        <CardButton icon="mdi-plus-circle" :text="$t('UserTestTable.buttons.createNewVariable')"
          @click="; dialog = true" />
      </v-col>
    </v-row>
  </v-container>
