...
*/
<template>
    <v-navigation-drawer v-model="isOpen" :width="drawerWidth" :temporary="isMobile"
        :permanent="!isMobile && isPermanent" :location="isMobile ? 'left' : 'right'" elevation="0" class="test-drawer">
        <!-- Test Header -->
        <div class="test-header pa-4">
            <div class="d-flex align-center justify-space-between">
                <div class="d-flex align-center">
                    ...
                    <div>
                        <h4 class="text-primary font-weight-bold">{{ testTitle }}</h4>
                        <p class="text-caption text-medium-emphasis ma-0">{{ testDescription }}</p>
                    </div>
                </div>
        ...
        <div class="drawer-content">
            ...
            <div v-if="showProgress" class="progress-section pa-4">
                ...
                <p class="text-caption text-medium-emphasis">{{ progressDescription }}</p>
            </div>
            ...
            <div class="action-buttons pa-4 mt-auto">
                <v-btn v-if="canGoBack" variant="outlined" color="primary" block class="mb-2"
                    prepend-icon="mdi-arrow-left" @click="goBack">
                    Previous Step
                </v-btn>

                <v-btn v-if="canGoNext" color="primary" block class="mb-2" append-icon="mdi-arrow-right"
                    @click="goNext">
                    Next Step
                </v-btn>
