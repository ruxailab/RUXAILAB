<template>
  <PageWrapper
    title="Automatic Accessibility Test Configuration"
    :loading="loadingPage"
  ...
  >
    ...
    <template #subtitle>
      <p class="text-body-1 text-grey-darken-1">
        Manage your automatic accessibility test settings and configuration with advanced controls
      </p>
    </template>
          ...
          >
            <div class="d-flex align-start ga-3 pa-6 pb-0">
              ...
              <div>
                ...
                <p class="text-caption text-grey-darken-1">
                  Test configuration and automation options
                </p>
              </div>
            </div>
            <v-card-text class="py-6">
...
const object = ref({
  ...
  version: '2.1',
  isPublic: false,
  testType: 'AUTOMATIC_ACCESSIBILITY',
});
...
const fetchTest = async () => {
  ...
  try {
    ...
    const fetchedTest = store.getters['automaticAccessibility/getTestById'](route.params.testId)
    
    if (fetchedTest) {
      // Map the fetched data to match FormTestDescription expectations
      object.value = {
        ...fetchedTest,
        testTitle: fetchedTest.title || '',
        testDescription: fetchedTest.description || '',
        testType: 'AUTOMATIC_ACCESSIBILITY',
        status: fetchedTest.status || 'draft',
        version: fetchedTest.version || '2.1',
        websiteUrl: fetchedTest.websiteUrl || '',
        isPublic: fetchedTest.isPublic || false,
      }
    } else {
      toast.error('Test not found')
      // Keep the default object values if test not found
      object.value = {
        ...object.value,
        title: '',
        testTitle: '',
...
const deleteTest = async item => {
  ...
  loading.value = true;
  try {
    await store.dispatch('automaticAccessibility/removeTest', item.id);
    toast.success('Test deleted successfully!');
    router.push({ name: 'AutomaticAccessibilityTests' });
  } catch (error) {
...
const updateObject = newObject => {
  ...
  // Map FormTestDescription properties back to AutomaticAccessibilityTest properties
  const mappedObject = {
    ...newObject,
...
const duplicateTest = async () => {
  if (!object.value) {
    toast.error('No test data to duplicate.');
    return;
  }
  
  loading.value = true;
  try {
    const testObj = new AutomaticAccessibilityTest({
      ...
      status: 'draft',
      version: object.value.version || '2.1',
      isPublic: false,
      id: null,
    ...
    await store.dispatch('automaticAccessibility/addTest', testObj);
