<template>
  <v-container class="pa-0 ma-0" fluid>
    ...
    <!-- LOGIC FOR WHEN USERS COME DIRECTLY TO THIS PAGE AND ARE PROMPT TO LOGIN OU SIGNUP -->
    ...
    <v-row class="nav pa-0 ma-0" dense v-if="test">
      ...
      <!-- DEAL WITH CHILDREN ROUTES OF DRAWER NAVIGATION -->
...
export default {
  ...
  methods: {
    ...
    goToRoute(routeName) {
      let route = {};
      if (routeName === "AnalyticsView") {
        route = {
          name: routeName,
          params: { id: this.test.id },
          query: { heuristic: null },
        };
      } else {
        route = {
          name: routeName,
    ...
    // },
    // signOut() {
    //     this.$store.dispatch("logoToRouteut").then(() => {
  ...
  computed: {
    isHomeView() {
      return this.$route.name === "ManagerView" ? true : false;
    },
    test() {
      ...
      if (this.test) {
        items = [
          ...
          },
          {
            ...
            routeName: "AnswerView",
          },
          {
            title: "Analytics",
            icon: "mdi-chart-bar",
            routeName: "AnalyticsView",
          },
        ];
  ...
  watch: {
    ...
    //                 });
    //             else if (
    //                 this.$store.getters.cooperators !==
