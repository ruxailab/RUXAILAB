                  ...
                  <v-col cols="10">
                    <v-row>
                      ...
                      <v-col v-if="ind == 0" cols="12">
                        <v-data-table :headers="heuristicsEvaluator.header" :items="heuristicsEvaluator.items"
                                      :items-per-page="15" class="elevation-1 cardStyle" dense
                        >
                          <template v-for="header in heuristicsEvaluator.header" v-slot:[`item.${header.value}`]="{
                            item,
                          }"
                          >
                            ...
                            <v-btn v-else :key="header.value" text @click="goToDataHeuristic(item.heuristic)">
                              {{ item[header.value] }}
                            </v-btn>
                          </template>
                        </v-data-table>
                      ...
                      <v-col v-if="ind == 1" cols="12">
                        <v-data-table :headers="heuristicsStatistics.header" :items="heuristicsStatistics.items"
                                      :items-per-page="15" class="elevation-1 cardStyle" dense
                        >
                          <template v-slot:item.percentage="{ item }">
                            <div style="padding-top:2px; padding-bottom:2px">
                              <v-chip style="width: 35%" :color="getColor(item.average, item.max, item.min)" dark>
                                {{ item.percentage }}
                              </v-chip>
                            </div>
                          </template>
