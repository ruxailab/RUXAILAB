                            ...
                            <v-col cols="10" v-if="ind == 0">
                                ...
                                >
                                    <template v-slot:item.result="{ item }">
                                        <v-chip
                                            :color="
                                                getColorPorcentage(item.result)
                                            "
                                            dark
                                            >{{ item.result }}%</v-chip
                                        >
    ...
    methods: {
        statistics() {
            ...
            answers.forEach((evaluator) => {
                ...
                evaluator.heuristics.forEach((heuristic) => {
                    ...
                    let noAplication = 0;
                    let noReply = 0;
                    let res = heuristic.questions.reduce(
        ...
        standardDeviation(array) {
            ...
            return Math.sqrt(
                array.reduce(
                    (total, valor) =>
                        total + Math.pow(average - valor, 2) / array.length,
                    0
                )
            );
        },
    ...
    computed: {
        heuristicsEvaluator() {
            ...
            if (this.resultEvaluator) {
                this.resultEvaluator.forEach((evaluator) => {
                    let header = table.header.find(
                        (h) => h.text == evaluator.id
                    );
                    if (!header) {
                        table.header.push({
                            text: evaluator.id,
                    ...
                    evaluator.heuristics.forEach((heuristic) => {
                        let item = table.items.find(
                            (i) => i.heuristic == heuristic.id
                        );
                        if (item) {
                            Object.assign(item, {
        ...
        finalResult() {
            ...
            if (this.evaluatorStatistics.items.length) {
                let res = this.evaluatorStatistics.items.reduce(
                    (total, value) => {
                        return (
                            total +
                            value.result / this.evaluatorStatistics.items.length
                        );
                    },
                    0
                );
