<template>
  <v-container
    fluid
    class="dashboard-container"
  >
    <!-- Header with User Welcome -->
    <div class="dashboard-header mb-6">
      <h1 class="text-h4 font-weight-bold text-grey-darken-4 mb-2">
        Welcome back, {{ userDisplayName }}! 👋
      </h1>
      <p class="text-subtitle-1 text-grey-darken-1">
        Here's what's happening with your research projects today
      </p>
    </div>

    <!-- Stats Cards Row -->
    <StatsCards
      :total-studies="totalStudies"
      :used-storage="usedStorage"
      :total-participants="totalParticipants"
    />

    <!-- Second Row: Activity Timeline and Active Studies -->
    <v-row class="mb-6">
      <v-col
        cols="12"
        lg="8"
      >
        <ActiveStudies :studies="items" />
      </v-col>
      <v-col
        cols="12"
        lg="4"
      >
        <ActivityTimeline />
      </v-col>
    </v-row>

    <!-- Third Row: Upcoming Webinar and Top Methods -->
    <v-row class="mb-6">
      <v-col
        cols="12"
        lg="4"
      >
        <UpcomingWebinar />
      </v-col>
      <v-col
        cols="12"
        lg="4"
      >
        <TopMethods />
      </v-col>
      <v-col
        cols="12"
        lg="4"
      >
        <NextSession />
      </v-col>
    </v-row>

    <!-- Fourth Row: Blog Posts and Next Session -->
    <v-row class="mb-6">
      <v-col
        cols="12"
        lg="6"
      >
        <BlogPosts />
      </v-col>
      <v-col
        cols="12"
        lg="6"
      />
    </v-row>
  </v-container>
</template>

<script setup>
import { ref, computed } from 'vue'
import { useStore } from 'vuex'
import StatsCards from '@/components/dashboard/StatsCards.vue'
import ActivityTimeline from '@/components/dashboard/ActivityTimeline.vue'
import ActiveStudies from '@/components/dashboard/ActiveStudies.vue'
import BlogPosts from '@/components/dashboard/BlogPosts.vue'
import UpcomingWebinar from '@/components/dashboard/UpcomingWebinar.vue'
import TopMethods from '@/components/dashboard/TopMethods.vue'
import NextSession from '@/components/dashboard/NextSession.vue'

const props = defineProps({
  items: {
    type: Array,
    required: true,
    default: () => [],
  },
})

const store = useStore()

// Dashboard data
const totalStudies = ref(12)
const usedStorage = ref(150)
const totalParticipants = ref(347)



// User info
const userDisplayName = computed(() => {
  const user = store.getters['auth/getUser']
  if (user && user.displayName) {
    return user.displayName.split(' ')[0]
  }
  return 'User'
})
</script>

<style scoped>
.dashboard-container {
  padding: 24px;
  background-color: rgb(var(--v-theme-background));
  min-height: 100vh;
}

.dashboard-header {
  text-align: left;
}

@media (max-width: 960px) {
  .dashboard-container {
    padding: 16px;
  }
}
</style>
