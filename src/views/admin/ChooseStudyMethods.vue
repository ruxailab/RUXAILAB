<template>
  <v-container fluid class="create-study-view">
    <v-container class="py-6">
      ...
      <v-row>
        <v-col cols="12">
          <v-stepper alt-labels :model-value="2" class="elevation-0 bg-transparent">
            <v-stepper-header class="elevation-0">
              <v-stepper-item :complete="true" :value="1" color="success" title="Category" />
              <v-divider />
              <v-stepper-item :complete="false" :value="2" color="primary" title="Methods" />
              <v-divider />
      ...
      <v-row justify="center">
        <v-col v-for="method in availableMethods" :key="method.id" cols="12" md="6" lg="5">
          <v-card :variant="selectedMethod === method.id ? 'tonal' : 'flat'"
            :color="selectedMethod === method.id ? 'primary' : undefined" :disabled="!method.available"
            ...
            <!-- Coming Soon Badge -->
            <v-chip v-if="method.comingSoon" color="warning" variant="flat" size="small" class="position-absolute"
            ...
            <v-card-text class="pa-8">
              <div class="d-flex align-center">
                <v-avatar :color="method.color" size="64" class="mr-4">
                  <v-icon :icon="method.icon" size="32" color="white" />
                </v-avatar>
              </div>
            ...
            <!-- Selected Indicator -->
            <v-icon v-if="selectedMethod === method.id" icon="mdi-check-circle" color="primary" size="large"
